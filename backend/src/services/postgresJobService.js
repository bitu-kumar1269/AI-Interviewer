...
async function getAllJobs() {
  ...
  // 2. Cache miss: Query Database (PostgreSQL if configured, otherwise MongoDB)
  console.log(`[Redis] Cache MISS (data served from Database) for key: ${cacheKey}`);
  let jobs = [];
  const mongoose = require('mongoose');

  try {
    const hasPgConfig = process.env.DATABASE_URL || process.env.PG_URL || process.env.PGHOST;
    if (hasPgConfig) {
      const res = await pool.query('SELECT * FROM jobs');
      jobs = res.rows.map(presentDbJob);
    } else {
      // Primary DB is MongoDB
      const Job = require('../models/Job.model');
      if (mongoose.connection.readyState === 1) {
        const dbJobs = await Job.find({ isActive: true }).sort({ postedTime: -1, createdAt: -1 }).lean();
        jobs = dbJobs.map(presentDbJob);
      } else {
        console.warn('[PostgresJobService] MongoDB is not connected yet.');
        jobs = [];
      }
    }
  } catch (err) {
    console.warn('[PostgresJobService] Primary query error, attempting MongoDB fallback:', err.message);
    try {
      const Job = require('../models/Job.model');
      if (mongoose.connection.readyState === 1) {
        const dbJobs = await Job.find({ isActive: true }).sort({ postedTime: -1, createdAt: -1 }).lean();
      ...
      console.error('[PostgresJobService] MongoDB query failed:', dbErr.message);
      jobs = [];
    }
  }
